...
class Div(Function):
    ...
    def backward(self, dLdf):
        ...
        self.t1.backward(grad)
        grad = (self.t1.data)*(-1/(self.t2.data**2)) * dLdf
        self.t2.backward(grad)
        #pass
...
class MatMul(Function):
    ...
    def backward(self, dLdf):
        ...
        grad = dLdf @ np.transpose(self.v.data)
        self.M.backward(grad)
        grad = np.transpose(self.M.data) @ dLdf
...
class ReLU(Function):
    ...
    def backward(self, dLdf):
        ...
        grad = ( np.greater(self.t.data, 0).astype(int) ) * dLdf
        self.t.backward(grad)        
        pass
...
class Sum(Function):
    ...
    def backward(self, dLdf):
        ...
        grad = np.ones_like(np.sum(self.t.data, axis=self.dim)) * dLdf
        self.t.backward(grad)       
        pass
...
class Mean(Function):
    ...
    def backward(self, dLdf):
        ...
        grad = np.ones_like(self.t.data) / self.t.data.size
        self.t.backward(grad)
        pass
